...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  // Tillfälliga säsongsteman: Halloween-dekorationer
  const [halloweenEnabled, setHalloweenEnabledState] = useState<boolean>(() => Boolean(settings.halloweenEnabled));
  const [isSavingHalloween, setIsSavingHalloween] = useState<boolean>(false);
  const [halloweenNotice, setHalloweenNotice] = useState<string | null>(null);

  useEffect(() => {
    setHalloweenEnabledState(Boolean(settings.halloweenEnabled));
  }, [settings.halloweenEnabled]);

  const handleToggleHalloween = async (newVal: boolean) => {
    setIsSavingHalloween(true);
    setHalloweenNotice(null);
    try {
      setHalloweenEnabledState(newVal);
      await onSaveSettings({ halloweenEnabled: newVal });
      setHalloweenNotice(
        newVal
          ? '🎃 Halloween-dekorationer har aktiverats på webbplatsen!'
          : '⚪ Halloween-dekorationer har stängts av.'
      );
      setTimeout(() => setHalloweenNotice(null), 4000);
    } catch (err) {
      setHalloweenEnabledState(!newVal);
      setHalloweenNotice('Kunde inte uppdatera Halloween-status.');
    } finally {
      setIsSavingHalloween(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSaveSettings({
        ...
        halloweenEnabled: Boolean(halloweenEnabled),
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
        ...
        {activeTab === 'status' && (
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            {/* 🎃 Tillfälligt Säsongstema: Halloween-dekorationer */}
            <div className="bg-[#FBF9F5] border border-[#E6DFD3] rounded-3xl p-6 sm:p-8 space-y-6">
              <div className="pb-4 border-b border-[#E6DFD3] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="font-serif text-2xl text-[#242D27] font-medium flex items-center gap-2.5">
                    <span className="text-xl">🎃</span>
                    <span>Halloween-dekorationer</span>
                  </h2>
                  <p className="text-xs text-[#66726A] font-light mt-1">
                    Styr de tillfälliga Halloween-dekorationerna på webbplatsen.
                  </p>
                </div>
                ...
                <div
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold border ${
                    halloweenEnabled
                      ? 'bg-[#FDF6F0] text-[#C86938] border-[#F2D7C8]'
                      : 'bg-[#F3EFE8] text-[#736B5E] border-[#E2DDD3]'
                  }`}
                >
                  <span
                    className={`w-2.5 h-2.5 rounded-full ${
                      halloweenEnabled ? 'bg-[#C86938] animate-pulse' : 'bg-[#9C9488]'
                    }`}
                  />
                  <span>{halloweenEnabled ? '🟢 Halloween aktiv' : '⚪ Halloween avstängd'}</span>
                </div>
              </div>

              {halloweenNotice && (
                <div className="p-3.5 rounded-2xl bg-[#EBF3EE] border border-[#CDE0D4] text-xs text-[#242D27] font-medium flex items-center justify-between animate-in fade-in duration-200">
                  <div className="flex items-center gap-2">
                    <span>{halloweenNotice}</span>
                  </div>
                  <button type="button" onClick={() => setHalloweenNotice(null)} className="text-[#66726A] hover:text-[#242D27]">
                    &times;
                  </button>
                </div>
              )}
              ...
              <div
                className={`p-6 sm:p-7 rounded-2xl border transition-all ${
                  halloweenEnabled
                    ? 'bg-[#FDF7F2] border-[#F0D5C5]'
                    : 'bg-[#FAF8F5] border-[#E6DFD3]'
                }`}
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
                  <div className="space-y-1.5">
                    <div className="text-xs font-semibold uppercase tracking-wider text-[#8C6D53]">
                      Tillfälligt säsongslager
                    </div>
                    <h3 className="font-serif text-xl sm:text-2xl text-[#242D27] font-semibold flex items-center gap-2">
                      <span>{halloweenEnabled ? '🎃 Halloween-dekorationer är synliga' : '⚪ Halloween-dekorationer är avstängda'}</span>
                    </h3>
                    <p className="text-xs text-[#526057] font-light leading-relaxed max-w-xl">
                      {halloweenEnabled
                        ? 'Mysiga, handgjorda dekorationer i dämpade hösttoner visas diskret i marginalerna på den publika webbplatsen. Inga knappar, texter, kassa eller produkter påverkas.'
                        : 'Webbplatsen visas i sitt helt ordinarie originalutseende utan några som helst dekorativa element.'}
                    </p>
                  </div>

                  {/* Snabb switch [ ON / OFF ] */}
                  <div className="flex-shrink-0 flex items-center bg-[#EBE5DB] p-1 rounded-xl border border-[#DCD5C9]">
                    <button
                      type="button"
                      disabled={isSavingHalloween}
                      onClick={() => handleToggleHalloween(true)}
                      className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                        halloweenEnabled
                          ? 'bg-[#C86938] text-[#FAF8F5] shadow-xs'
                          : 'text-[#66726A] hover:text-[#242D27]'
                      }`}
                    >
                      <span>ON</span>
                      {halloweenEnabled && <span className="w-1.5 h-1.5 rounded-full bg-white animate-ping" />}
                    </button>
                    <button
                      type="button"
                      disabled={isSavingHalloween}
                      onClick={() => handleToggleHalloween(false)}
                      className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                        !halloweenEnabled
                          ? 'bg-[#FAF8F5] text-[#242D27] shadow-xs'
                          : 'text-[#66726A] hover:text-[#242D27]'
                      }`}
                    >
                      OFF
              ...
              {/* Tydliga interaktiva val för ON / OFF */}
              <div className="space-y-3 pt-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl">
                  ...
                  <button
                    ...
                    className={`flex items-start gap-3 p-4 rounded-xl border text-left transition-all cursor-pointer ${
                      halloweenEnabled
                        ? 'bg-[#FDF7F2] border-[#C86938] ring-2 ring-[#C86938]/20 shadow-xs'
                        : 'bg-[#FAF8F5] border-[#E6DFD3] hover:border-[#C86938]/50'
                    }`}
                  >
                    <span className="text-xl leading-none mt-0.5">🎃</span>
                    <div>
                      <div className="text-xs font-semibold text-[#242D27] flex items-center gap-1.5">
                        <span>Aktivera Halloween (ON)</span>
                        {halloweenEnabled && <span className="text-[10px] text-[#C86938] font-bold">● Aktiv</span>}
                      </div>
                      <div className="text-[11px] text-[#66726A] font-light mt-0.5">
                        Visar diskreta, virkade minipumpor, små stjärnor, fladdermöss och spöken i marginalerna.
                      </div>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className={`flex items-start gap-3 p-4 rounded-xl border text-left transition-all cursor-pointer ${
                      !halloweenEnabled
                        ? 'bg-[#F4F1EA] border-[#8C6D53] ring-2 ring-[#8C6D53]/20 shadow-xs'
                        : 'bg-[#FAF8F5] border-[#E6DFD3] hover:border-[#8C6D53]/50'
                    }`}
                  >
                    <span className="text-xl leading-none mt-0.5">⚪</span>
                    <div>
                      <div className="text-xs font-semibold text-[#242D27] flex items-center gap-1.5">
                        <span>Stäng av Halloween (OFF)</span>
                        {!halloweenEnabled && <span className="text-[10px] text-[#8C6D53] font-bold">● Aktiv</span>}
                      </div>
                      <div className="text-[11px] text-[#66726A] font-light mt-0.5">
                        Alla Halloween-dekorationer tas bort. Sagomaskan visas exakt som i vanligt standardläge.
                      </div>
                    </div>
                  </button>
                </div>
              ...
              {/* Information om separat säsongstema */}
              <div className="p-4 rounded-xl bg-[#FAF8F5] border border-[#E6DFD3] text-xs text-[#66726A] space-y-1">
                <div className="font-semibold text-[#242D27] flex items-center gap-1.5">
                  <span className="text-sm">ℹ️</span>
                  <span>Oberoende tillfällig funktion</span>
                </div>
                <p className="font-light leading-relaxed">
                  Halloween-inställningen är helt fristående från underhållsläget och påverkar varken produkter, priser, kundvagn eller ordersystem. Statusen sparas i realtid i databasen och kan stängas av när som helst med ett klick.
                </p>
              </div>
            </div>
          </div>
